    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        <Stat value={data.allTimeJobs.toLocaleString()} label="jobs all time"
              detail={`${data.allTimeFilled.toLocaleString()} filled · ${allTimeRate}%`} tone="text-zinc-200" />
        <Stat value={data.openNow.toLocaleString()} label="open right now"
              detail={`${data.openUnfilled} still need somebody`} tone="text-cyan-300" />
        <Stat value={data.activeCandidates.toLocaleString()} label="active candidates"
              detail="on the market, never placed" tone="text-emerald-300" />
      </div>

      <p className="mt-5 text-[14px] leading-relaxed text-zinc-300">
        {last && (
          <>
            <span className="font-semibold text-zinc-100">{last.opened} jobs</span> arrived in{' '}
            {monthLabel(last.month)}, against {months[0]?.opened ?? 0} in {monthLabel(months[0]?.month ?? '')}.
            {' '}We fill about{' '}
            <span className="font-semibold text-amber-300">{recentRate}%</span> of what comes in.
          </>
        )}
      </p>
          ...
          {(['time', 'specialty'] as const).map(k => (
            <button key={k} onClick={() => setBy(k)}
                    className={cn('rounded-md px-3 py-1 text-[11px] font-medium transition-colors',
                      by === k ? 'bg-white/10 text-zinc-100' : 'text-zinc-500 hover:text-zinc-300')}>
        ...
        {months.map(m => {
          ...
          return (
            <div key={m.month} className={cn('flex items-center gap-3', wip && 'opacity-70')}>
              <span className="w-12 shrink-0 text-[12px] text-zinc-400">{monthLabel(m.month)}</span>
              ...
              <span className="relative h-5 grow rounded bg-zinc-800/40">
                <span className="absolute inset-y-0 left-0 rounded bg-zinc-700"
              ...
              <span className="w-28 shrink-0 text-right text-[11px] tabular-nums text-zinc-400">
                <span className="text-emerald-300">{m.filled}</span> of {m.opened}
              </span>
              <span className={cn('w-16 shrink-0 text-right text-[12px] font-semibold tabular-nums',
                rate >= 30 ? 'text-emerald-300' : rate > 0 ? 'text-amber-300' : 'text-zinc-600')}>
                {rate}%
              </span>
            </div>
          )
...
function SpecialtyView({ data }: { data: SupplyDemandData }) {
  ...
  return (
    <div>
      {failing.length > 0 && (
        <p className="mb-3 rounded-md border border-red-500/25 bg-red-500/[0.07] px-3 py-2 text-[12px] leading-relaxed text-red-100">
        ...
        {rows.map(s => {
          ...
          return (
            <div key={s.specialty} className="flex items-center gap-3">
              <span className="w-40 shrink-0 truncate text-[12px] text-zinc-300" title={s.specialty}>
                {s.specialty}
              </span>
              <span className="relative h-5 grow rounded bg-zinc-800/40">
                <span className="absolute inset-y-0 left-0 rounded bg-zinc-700"
              ...
              <span className="w-24 shrink-0 text-right text-[11px] tabular-nums text-zinc-400">
                <span className={s.filled ? 'text-emerald-300' : 'text-red-400'}>{s.filled}</span> of {s.opened}
              </span>
              <span className={cn('w-14 shrink-0 text-right text-[12px] font-semibold tabular-nums',
                rate >= 30 ? 'text-emerald-300' : rate > 0 ? 'text-amber-300' : 'text-red-400')}>
                {rate}%
              </span>
...
function Stat({ value, label, detail, tone }: { value: string; label: string; detail: string; tone: string }) {
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
      ...
      <p className="mt-1.5 text-[12px] text-zinc-300">{label}</p>
