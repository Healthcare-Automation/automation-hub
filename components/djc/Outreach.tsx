    ...
    <div className="space-y-9">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat value={String(attempted)} label="someone tried to reach"
              detail={`of ${base} sourced`} tone="text-cyan-300" />
        <Stat value={String(spoke)} label="ever actually spoke to us"
              detail="a two-way conversation was logged" tone="text-teal-300" />
        <Stat value={String(detail.onlyFailed)} label="tried but never reached"
              detail="texted or called, no response at all" tone="text-orange-300" />
        <Stat value={`${detail.convThenForward} of ${forwarded}`} label="put forward had spoken first"
              detail="a conversation nearly always precedes it" tone="text-zinc-200" />
      </div>

      <section>
        <h3 className="text-[13px] font-semibold text-zinc-100">Trying is not the same as reaching</h3>
            ...
            return (
              <div key={s.label}>
                {prev && lost > 0 && (
                  <div className="flex items-center gap-3 py-1">
                    ...
                    <span className="flex grow items-center gap-2">
                      <span className="text-[10px] tabular-nums text-orange-300/60">−{lost}</span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="w-16 shrink-0 text-right text-[13px] font-semibold tabular-nums text-zinc-100">
                    {s.people}
                  </span>
                </div>
              </div>
        ...
        <p className="mt-4 max-w-3xl text-[13px] leading-relaxed text-zinc-300">
      ...
      <section>
        <h3 className="text-[13px] font-semibold text-zinc-100">Which channel actually lands</h3>
        <div className="mt-3 grid gap-3 lg:grid-cols-3">
          <Channel label="Text" value={detail.smsSent.toLocaleString()} tone="text-cyan-300"
                   detail={`${smsReadRate}% opened. The workhorse, and the only channel with a read receipt.`} />
          <Channel label="Phone" value={detail.conversations.toLocaleString()} tone="text-teal-300"
                   detail="conversations logged. This is the screening step — a placement almost never happens without one." />
          <Channel label="Email" value={detail.email.sent.toLocaleString()} tone="text-zinc-200"
      ...
      {months.length > 0 && (
        <section>
          <h3 className="text-[13px] font-semibold text-zinc-100">Is anyone getting to the new ones?</h3>
            ...
            {months.map(m => {
              ...
              return (
                <div key={m.month} className="flex items-center gap-3">
                  <span className="w-28 shrink-0 text-[12px] text-zinc-400">
                  ...
                  <span className="w-14 shrink-0 text-right text-[13px] font-semibold tabular-nums text-zinc-200">
                    {pct}%
                  </span>
                  <span className="w-32 shrink-0 text-right text-[11px] tabular-nums text-zinc-600">
      ...
      <section className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4">
        <h3 className="text-[12px] font-semibold text-zinc-200">What this data cannot tell us</h3>
        <ul className="mt-2 space-y-1.5 text-[11px] leading-relaxed text-zinc-500">
          <li>
            <span className="text-zinc-400">Why a recruiter stopped.</span> One attempt then silence
            could be a bad number, a candidate who declined, or a queue nobody got back to. The log
            records the attempt, never the reason.
          </li>
          <li>
            <span className="text-zinc-400">What was said.</span> A logged conversation might have
            ended in &ldquo;not interested&rdquo; or &ldquo;call me next month&rdquo;. Both look identical here.
          </li>
          <li>
            <span className="text-zinc-400">Whether more calls cause more placements.</span> Candidates
          ...
          <li>
            <span className="text-zinc-400">Anything outside Salesforce.</span> A call from a personal
