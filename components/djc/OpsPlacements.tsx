  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat value={ops.avgPerMonth.toFixed(1)} label="placements a month" detail="2026 so far" tone="text-cyan-300" />
        <Stat value={ops.avgPerMonthPriorYear.toFixed(1)} label="a month last year"
              detail="same months of 2025" tone="text-zinc-300" />
        <Stat value={ops.ytd.toLocaleString()} label="placed year to date" detail={`through ${ops.monthsElapsed} months`} tone="text-zinc-100" />
...
function Bars({
  ...
  return (
    <div className="overflow-x-auto pb-1">
            ...
            >
              ...
              <div className="flex w-full items-end justify-center gap-1" style={{ height: H }}>
                ...
                <div className="relative w-[62%]"
                  ...
                  <span className="absolute -top-[18px] left-1/2 -translate-x-1/2 whitespace-nowrap text-[12px] font-semibold tabular-nums text-zinc-100">
                    {r.now}
                  </span>
                <div
      ...
      <p className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-zinc-600">
        ...
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-3 rounded-sm border border-dashed border-zinc-500" />
          still in progress
        </span>
      </p>
    </div>
        ...
        <tbody>
          {rows.map(r => (
            <tr key={r.name} className="border-t border-zinc-800/70">
              <td className="max-w-56 truncate px-3 py-1.5 text-zinc-200" title={r.name}>{r.name}</td>
              <td className="py-1.5 pr-3">
                <span className={cn('block h-2 rounded-full', CHART.primary)}
                      style={{ width: `${Math.max((r.placed / max) * 100, 2)}%` }} />
              </td>
              <td className="py-1.5 pr-3 text-right tabular-nums text-zinc-200">{r.placed}</td>
