    ...
    <div>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat value={data.ytdOpened.toLocaleString()} label="jobs opened this year"
              detail={`${data.priorYtdOpened} by this point last year`} tone="text-cyan-300" />
        <Stat value={`${subRate}%`} label="had someone put forward"
              detail={`${data.ytdSubmitted} of ${data.ytdOpened}`} tone="text-zinc-200" />
        <Stat value={`${fillRate}%`} label="were filled"
              detail={`${data.ytdFilled} of ${data.ytdOpened}`} tone="text-teal-300" />
        <Stat value={data.openNow.toLocaleString()} label="open right now"
              detail={`across ${data.practicesTotal} practices`} tone="text-zinc-200" />
      </div>

      <div className="mt-5 mb-3 flex flex-wrap items-center justify-between gap-2">
        <span className="inline-flex flex-wrap rounded-lg border border-zinc-700/60 bg-zinc-800/40 p-0.5">
          {VIEWS.map(v => (
            <button key={v.key} onClick={() => setView(v.key)}
                    className={cn('rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors',
                      view === v.key ? 'bg-white/10 text-zinc-100' : 'text-zinc-500 hover:text-zinc-300')}>
              {v.label}
            </button>
          ))}
        </span>
        <button onClick={() => setAsTable(v => !v)}
                className="rounded-lg border border-zinc-700/60 bg-zinc-800/40 px-2.5 py-1 text-[11px] font-medium text-zinc-400 transition-colors hover:text-zinc-200">
          {asTable ? 'Chart' : 'Table'}
        </button>
      </div>
      ...
      <p className="mt-5 border-t border-zinc-800 pt-3 text-[11px] leading-relaxed text-zinc-500">
            ...
            {rows.map(r => (
              <tr key={r.label} className="border-t border-zinc-800/70">
                <td className="px-3 py-1.5 text-zinc-300">
                  {r.label}{r.partial && <span className="ml-1 text-[10px] text-zinc-600">so far</span>}
                </td>
                <td className="py-1.5 pr-3 text-right tabular-nums text-zinc-200">{r.opened}</td>
                <td className="py-1.5 pr-3 text-right tabular-nums text-zinc-300">{r.submitted}</td>
    ...
    <div className="space-y-2">
      {rows.map(r => (
        <div key={r.label} className="flex items-center gap-3">
          <span className="w-36 shrink-0 truncate text-[12px] text-zinc-300" title={r.label}>
            {r.label}{r.partial && <span className="ml-1 text-[10px] text-zinc-600">so far</span>}
          </span>
          ...
          <span className={cn('relative h-6 grow rounded', CHART.track)}>
            <span className={cn('absolute inset-y-0 left-0 rounded',
              r.partial ? 'bg-cyan-400/20' : 'bg-cyan-400/30')}
            ...
            {r.submitted > 0 && (
              <span className="absolute inset-y-0 w-[2px] rounded-full bg-zinc-200/70"
          ...
          <span className="w-44 shrink-0 text-right text-[11px] tabular-nums text-zinc-500">
            <span className="text-zinc-200">{r.opened}</span> opened ·{' '}
            <span className="text-zinc-300">{r.submitted}</span> forward ·{' '}
            <span className="text-teal-300">{r.filled}</span> filled
          </span>
        </div>
      ))}
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 pt-1 text-[10px] text-zinc-500">
        <Key tone="bg-cyan-400/30" label="jobs opened" />
        <Key tone={CHART.good} label="of those, filled" />
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-[2px] rounded-full bg-zinc-200/70" /> someone put forward
        </span>
      </div>
    </div>
  )
        ...
        {data.durations.map(d => {
          ...
          return (
            <div key={d.label} className="flex items-center gap-3">
              <span className="w-32 shrink-0 text-[12px] text-zinc-300">{d.label}</span>
              <span className={cn('relative h-6 grow rounded', CHART.track)}>
                <span className="absolute inset-y-0 left-0 rounded bg-cyan-400/30"
                      style={{ width: `${Math.max((d.jobs / max) * 100, 0.8)}%` }} />
                <span className={cn('absolute inset-y-1 left-0 rounded-sm', CHART.good)}
                      style={{ width: `${(d.filled / max) * 100}%` }} />
              </span>
              <span className="w-16 shrink-0 text-right text-[12px] tabular-nums text-zinc-200">{d.jobs}</span>
              <span className="w-12 shrink-0 text-right text-[11px] tabular-nums text-zinc-600">{d.pct}%</span>
              <span className={cn('w-24 shrink-0 text-right text-[12px] font-semibold tabular-nums',
                rate >= 30 ? 'text-teal-300' : 'text-zinc-400')}>
                {rate}% filled
              </span>
            </div>
          )
  ...
  return (
    <div>
      <p className="mb-3 max-w-3xl text-[12px] leading-relaxed text-zinc-400">
        ...
        <span className="text-orange-300">{stale} have been open over three months</span> — the
        clients most likely to be losing patience.
      </p>
      <div className="space-y-2">
        {data.openAges.map(a => (
          <div key={a.label} className="flex items-center gap-3">
            <span className="w-32 shrink-0 text-[12px] text-zinc-300">{a.label}</span>
            <span className={cn('relative h-6 grow rounded', CHART.track)}>
              <span className={cn('absolute inset-y-0 left-0 rounded',
                a.label === 'Over 3 months' ? CHART.warn : CHART.primary)}
                    style={{ width: `${Math.max((a.jobs / max) * 100, 1)}%` }} />
            </span>
            <span className="w-16 shrink-0 text-right text-[13px] font-semibold tabular-nums text-zinc-200">
              {a.jobs}
            </span>
          </div>
        ))}
...
function Stat({ value, label, detail, tone }: { value: string; label: string; detail: string; tone: string }) {
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
      ...
      <p className="mt-1.5 text-[12px] leading-tight text-zinc-300">{label}</p>
      <p className="mt-0.5 text-[11px] leading-tight text-zinc-600">{detail}</p>
    </div>
  )
}
