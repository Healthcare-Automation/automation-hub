        ...
        {failed ? (
          ...
          <>
            <div className="shrink-0 border-b border-zinc-200 dark:border-zinc-800 px-5 py-4">
              <div className="flex items-end gap-6">
                ...
                <div className="mb-0.5 flex gap-5">
                  {data.facts.map(f => (
                    <div key={f.label}>
                      <p className="text-[15px] font-semibold tabular-nums text-zinc-800 dark:text-zinc-200">{f.value}</p>
                      <p className="mt-0.5 text-[10px] leading-tight text-zinc-500">{f.label}</p>
                      {f.hint && <p className="text-[10px] leading-tight text-zinc-400 dark:text-zinc-600">{f.hint}</p>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              {data.rows.length === 0 ? (
                <p className="px-5 py-10 text-center text-[12px] text-zinc-500 dark:text-zinc-600">Nothing here.</p>
              ) : (
                  ...
                  <tbody>
                    {data.rows.map((r, i) => (
                      <tr key={i} className="border-t border-zinc-200 hover:bg-zinc-50 dark:border-zinc-800/70 dark:hover:bg-zinc-800/30">
                        <td className="px-5 py-2 font-medium text-zinc-800 dark:text-zinc-200">
                          ...
                          {r.sfId ? (
                            <a href={`${SF_CONTACT}/${r.sfId}/view`} target="_blank" rel="noreferrer"
                               ...
                               className="text-zinc-900 decoration-zinc-300 hover:text-cyan-700 dark:text-zinc-100 dark:decoration-zinc-700 dark:hover:text-cyan-300 underline underline-offset-2 transition-colors hover:decoration-cyan-500">
                              {r.name ?? r.sfId}
                              <span className="ml-1 text-[9px] text-zinc-400 dark:text-zinc-600">↗</span>
                            </a>
                          ) : (
                            (r.name ?? '—')
                          )}
                        ...
                        <td className="max-w-64 py-2 pr-3 text-zinc-500 dark:text-zinc-500">
                          <span className="block truncate" title={r.job ?? undefined}>
                            {showJob ? (r.job ?? '—') : (r.note ?? '—')}
                          </span>
                          {showJob && r.note && (
                            <span className="block truncate text-[10px] text-zinc-400 dark:text-zinc-600">{r.note}</span>
                          )}
                        </td>
