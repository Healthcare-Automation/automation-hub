  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-3 text-[11px] text-zinc-500">
        <span className={runOk ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}>
          ● {runOk ? 'Automation healthy' : `Last run: ${data.lastRun.status ?? 'unknown'}`}
        </span>
        <span>last sync {data.lastRun.at ?? '—'} ET · refreshes hourly</span>
        {data.conserveActive && (
          <span
            className="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-0.5 text-cyan-700 dark:text-cyan-300"
      ...
      >
        ...
        <Link href="/djc/acquisition" className="mt-3 inline-block text-[11px] text-cyan-600 dark:text-cyan-400 hover:underline">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        >
          <ColumnChart
            ...
            color={EMERALD}
          />
          <Link href="/djc/candidates" className="mt-3 inline-block text-[11px] text-cyan-600 dark:text-cyan-400 hover:underline">
      ...
      {kimedics && (
        <Link
          href="/impact"
          className="block rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/40 dark:bg-zinc-900/40 dark:shadow-none p-4 transition-colors hover:border-emerald-300 hover:bg-zinc-50 dark:hover:border-emerald-800/50 dark:hover:bg-zinc-800/40 sm:p-5"
        >
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <p className="text-[13px] font-semibold text-zinc-900 dark:text-zinc-100">Kimedics automation</p>
            <span className="text-[11px] text-emerald-600 dark:text-emerald-400">full impact analysis →</span>
          </div>
          <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-4">
            <KimStat v={`${kimedics.hoursSaved} hrs`} l="manual work returned" accent="text-emerald-700 dark:text-emerald-300" />
            ...
            <KimStat
              ...
              accent={kimedics.failuresThisMonth === 0 ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300'}
            />
          </div>
        </Link>
      )}
...
function ExecSummary({ initial }: { initial: { id: number; text: string; generatedAt: string } }) {
  ...
  return (
    <div className="rounded-xl border border-cyan-200 bg-gradient-to-br from-cyan-50 to-white shadow-sm dark:border-cyan-800/40 dark:from-cyan-950/30 dark:to-zinc-900/40 dark:shadow-none p-5">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <p className="text-[13px] font-semibold text-zinc-900 dark:text-zinc-100">This week in review</p>
        <div className="flex items-center gap-3 text-[10.5px] text-zinc-500">
          ...
          <button
            onClick={refresh}
            disabled={state === 'busy'}
            className="rounded-md border border-zinc-300 dark:border-zinc-600/60 px-2 py-0.5 text-[10.5px] text-zinc-700 dark:text-zinc-300 transition-colors hover:border-cyan-600 hover:text-cyan-700 dark:hover:text-cyan-300 disabled:cursor-wait disabled:opacity-50"
          >
            {state === 'busy' ? 'Rewriting… ~30s' : 'Refresh'}
          </button>
        </div>
      </div>
      <p className="mt-2 text-[13px] leading-relaxed text-zinc-700 dark:text-zinc-300">{summary.text}</p>
      {state === 'error' && (
        <p className="mt-1 text-[10.5px] text-amber-600 dark:text-amber-400">Refresh didn&apos;t complete — try again in a minute.</p>
      )}
    </div>
  )
}
...
function Hero({
  ...
  return (
    <Link
      href={href}
      className="min-w-0 rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/50 dark:bg-zinc-800/30 dark:shadow-none p-5 transition-colors hover:border-zinc-300 hover:bg-zinc-50 dark:hover:border-zinc-600 dark:hover:bg-zinc-800/60"
    >
...
function KimStat({ v, l, accent }: { v: string; l: string; accent?: string }) {
  return (
    <div>
      <div className={`text-lg font-semibold tabular-nums ${accent ?? 'text-zinc-900 dark:text-zinc-100'}`}>{v}</div>
...
function DigCard({ href, title, text }: { href: string; title: string; text: string }) {
  return (
    <Link
      href={href}
      className="rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/40 dark:bg-zinc-900/40 dark:shadow-none p-4 transition-colors hover:border-cyan-300 hover:bg-zinc-50 dark:hover:border-cyan-800/50 dark:hover:bg-zinc-800/40"
    >
