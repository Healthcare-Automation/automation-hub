...
export default function ViewSpend({ activity }: { activity: ActivityBucket[] }) {
  ...
  return (
    <section>
      <h3 className="text-[13px] font-semibold text-zinc-100">When candidates were last active on DJC</h3>
      ...
      <div className="mt-4 space-y-1.5">
        {activity.map(b => (
          <div key={b.label} className="flex items-center gap-3">
            <span className="w-36 shrink-0 text-[12px] text-zinc-300">{b.label}</span>
            ...
            <span className="w-16 shrink-0 text-right text-[12px] font-semibold tabular-nums text-zinc-200">
