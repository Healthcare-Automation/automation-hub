...
function Section({
  ...
  return (
    <section className="border-t border-zinc-200 dark:border-zinc-800 pt-8 first:border-0 first:pt-0">
      <div className="mb-5 flex items-baseline gap-3">
        <span className="text-[11px] font-semibold tabular-nums text-zinc-500 dark:text-zinc-600">0{n}</span>
        <div>
  ...
  return (
    <div className="space-y-10">
      <header className="max-w-3xl">
        <h1 className="text-[20px] font-semibold text-zinc-900 dark:text-zinc-100">How the business is doing</h1>
      ...
      <Section n={3} title="The pipeline"
        ...
        <p className="mb-4 text-[12px] leading-relaxed text-zinc-500">
          Each row below counts a <span className="text-zinc-700 dark:text-zinc-300">candidate–job pairing</span>, not a
          person: {apps.toLocaleString()} pairings across{' '}
          <span className="text-zinc-700 dark:text-zinc-300">{funnelMeta.people.toLocaleString()} people</span> and{' '}
          ...
          <span className="text-zinc-500 dark:text-zinc-600">Click any row to see the people in it.</span>
        </p>
        <div className="space-y-2.5">
          {([
            { label: 'Put forward for a job', n: apps, of: apps, tone: 'bg-zinc-400 dark:bg-zinc-500/50', stage: 'apps' as const },
            ...
            <button key={s.label} type="button"
                    ...
                    className="block w-full rounded-md px-1 py-0.5 text-left transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800/40">
              <div className="flex items-baseline justify-between text-[12px]">
                <span className="text-zinc-700 dark:text-zinc-300">{s.label}</span>
                <span className="tabular-nums text-zinc-600 dark:text-zinc-400">
                  {s.n.toLocaleString()}{' '}
                  <span className="text-zinc-500 dark:text-zinc-600">({Math.round((s.n / (s.of || 1)) * 100)}%)</span>
                </span>
              </div>
