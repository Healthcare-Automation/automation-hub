...
export default function FunnelDrill({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-6"
         onClick={onClose}>
      <div
        className="flex max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-t-2xl border border-zinc-700 bg-zinc-900 shadow-2xl sm:rounded-2xl"
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-auto">
          {failed ? (
            <p className="px-5 py-8 text-center text-[12px] text-amber-300/80">
              The database was busy. Close and try again in a few seconds.
            </p>
          ) : rows === null ? (
            <p className="px-5 py-8 text-center text-[12px] text-zinc-600">Loading…</p>
          ) : rows.length === 0 ? (
            <p className="px-5 py-8 text-center text-[12px] text-zinc-600">
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className="border-t border-zinc-800/70 hover:bg-zinc-800/30">
                    <td className="whitespace-nowrap px-5 py-2 font-medium text-zinc-200">
                    ...
                    <td className="whitespace-nowrap py-2 pr-4 text-zinc-400">{r.stage ?? '—'}</td>
