...
export default function DjcViewEfficiencyChart({
  ...
  if (!active.length) {
    return <p className="text-xs text-zinc-600">No Profile View activity in this window.</p>
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-end gap-x-8 gap-y-3">
        <div>
          <p className="text-[30px] leading-none font-semibold tabular-nums text-cyan-300">{latestRate}%</p>
        ...
        {delta !== null && (
          <div>
            <p className={'text-[15px] font-semibold tabular-nums ' +
                          (delta > 0 ? 'text-emerald-300' : delta < 0 ? 'text-amber-300' : 'text-zinc-400')}>
        ...
        <div>
          <p className="text-[15px] font-semibold tabular-nums text-zinc-300">{overall}%</p>
          <p className="mt-1 text-[11px] text-zinc-500">
      ...
      <div className="h-9">
        {shown ? (
          <div className="inline-flex flex-wrap items-baseline gap-x-3 gap-y-0.5 rounded-md bg-zinc-800/70 px-3 py-1.5 ring-1 ring-zinc-700/60">
            <span className="text-[12px] font-medium text-zinc-200">{label(shown.day)}</span>
            <span className="text-[11px] text-amber-300">{shown.views} view{shown.views === 1 ? '' : 's'} spent</span>
            <span className="text-[11px] text-cyan-300">{shown.created} added</span>
            <span className="text-[11px] font-semibold text-zinc-200">{rate(shown)}%</span>
            {shown.freeSkips > 0 && (
              <span className="text-[11px] text-emerald-300">{shown.freeSkips} caught free</span>
            )}
          </div>
        ) : (
          <p className="text-[11px] text-zinc-600">Hover for the exact numbers.</p>
        )}
      </div>
        ...
        <svg width={PAD_L} height={H} className="block shrink-0">
          {[0, 25, 50, 75, 100].map(g => (
            <text key={g} x={PAD_L - 6} y={y(g) + 3} textAnchor="end"
                  className="fill-zinc-500 text-[9px] tabular-nums">{g}%</text>
          ))}
        </svg>
        ...
        <svg width={width} height={H} className="block">
          {[0, 25, 50, 75, 100].map(g => (
            <line key={g} x1={0} x2={width - PAD_R} y1={y(g)} y2={y(g)} stroke="currentColor"
                  className="text-zinc-700/30" strokeDasharray="2 3" />
          ))}
          ...
          {active.map((d, i) => {
            ...
            return (
              <rect key={d.day} x={cx(i) - (COL - 18) / 2} y={PAD_T + PLOT_H - h}
                    ...
                    className={hover === i ? 'fill-amber-400/45' : 'fill-amber-500/20'} />
            )
          })}

          <polyline points={linePoints} fill="none" strokeWidth={2} strokeLinejoin="round"
                    strokeLinecap="round" className="stroke-cyan-400/90" />
          ...
          {active.map((d, i) =>
            (i % labelEvery === 0 || hover === i) ? (
              <text key={`pct-${d.day}`} x={cx(i)} y={y(rate(d)) - 8} textAnchor="middle"
                    className={(hover === i ? 'fill-cyan-200 font-semibold' : 'fill-cyan-300/70') +
          ...
          {active.map((d, i) => (
            <text key={`lbl-${d.day}`} x={cx(i)} y={H - 8} textAnchor="middle"
                  className={(hover === i ? 'fill-zinc-300' : 'fill-zinc-600') + ' text-[9px]'}>
              {label(d.day)}
            </text>
          ))}
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-zinc-500">
        <span className="flex items-center gap-1.5">
          <span className="h-2 w-3 rounded-sm bg-amber-500/25" /> Profile Views spent (volume)
        </span>
        <span className="flex items-center gap-1.5">
          <span className="h-0.5 w-4 rounded-full bg-cyan-400/90" /> share that became a candidate
        </span>
      </div>
    </div>
  )
