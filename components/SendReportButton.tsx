...
const ACCENT: Record<Accent, { trigger: string; focus: string; chip: string; send: string; ring: string; glow: string }> = {
  emerald: {
    trigger: 'text-emerald-300/90 ring-emerald-500/25 hover:bg-emerald-500/10 hover:text-emerald-200',
    focus: 'focus-within:ring-emerald-500/50',
    chip: 'bg-emerald-500/15 text-emerald-100 ring-emerald-500/25',
    send: 'bg-emerald-500 text-emerald-950 hover:bg-emerald-400',
    ring: 'ring-emerald-500/20',
    glow: 'from-emerald-500/15',
  },
  cyan: {
    trigger: 'text-cyan-300/90 ring-cyan-500/25 hover:bg-cyan-500/10 hover:text-cyan-200',
    focus: 'focus-within:ring-cyan-500/50',
    chip: 'bg-cyan-500/15 text-cyan-100 ring-cyan-500/25',
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm animate-[fadeIn_.15s_ease-out]" onClick={close} />
          <div className={`relative w-full max-w-md overflow-hidden rounded-2xl bg-zinc-900 ring-1 ${a.ring} shadow-2xl animate-[popIn_.18s_ease-out]`}>
            <div className={`pointer-events-none absolute inset-x-0 top-0 h-24 bg-gradient-to-b ${a.glow} to-transparent`} />
            <div className="relative p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-[15px] font-semibold text-white">Send all-time report</h3>
                  <p className="mt-0.5 text-xs text-zinc-400">{title}</p>
                </div>
                <button onClick={close} disabled={sending} className="rounded-md p-1 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-300 disabled:opacity-40">
              ...
              {status === 'done' ? (
                ...
                <>
                  <div className="mt-4">
                    ...
                    <div className={`mt-1.5 flex flex-wrap items-center gap-1.5 rounded-lg bg-zinc-800/60 p-2 ring-1 ring-zinc-700/70 transition-shadow ${a.focus}`}>
                      {emails.map((e, i) => (
                        <span key={e} className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[12px] ring-1 ${a.chip}`}>
                          {e}
                          <button onClick={() => setEmails((p) => p.filter((_, j) => j !== i))} disabled={sending} className="text-current/70 hover:text-white">
                      ...
                      <input
                        ...
                        className="min-w-[140px] flex-1 bg-transparent px-1 py-0.5 text-[13px] text-zinc-100 outline-none placeholder:text-zinc-600"
                      />
                    </div>
                    <p className="mt-1.5 text-[11px] text-zinc-600">Press Enter or comma to add. Multiple recipients allowed.</p>
                  </div>

                  {status === 'error' && msg && (
                    <p className="mt-3 rounded-md bg-red-500/10 px-3 py-2 text-[12px] text-red-300 ring-1 ring-red-500/20">{msg}</p>
                  )}

                  <div className="mt-5 flex items-center justify-end gap-2">
                    <button onClick={close} disabled={sending} className="rounded-lg px-3 py-1.5 text-[13px] text-zinc-400 transition-colors hover:text-zinc-200 disabled:opacity-40">Cancel</button>
                    <button
                      onClick={send}
