  ...
  return (
    <div className="space-y-10">
      <header className="max-w-3xl">
        <h1 className="text-[20px] font-semibold text-zinc-900 dark:text-zinc-100">What the automations have produced</h1>
      ...
      <div className="rounded-xl border border-cyan-200 bg-gradient-to-br from-cyan-50 to-white shadow-sm dark:border-cyan-800/40 dark:from-cyan-950/30 dark:to-zinc-900/40 dark:shadow-none p-5">
        <p className="text-[13px] font-semibold text-zinc-900 dark:text-zinc-100">The whole story, in four sentences</p>
        <ol className="mt-2 max-w-3xl list-decimal space-y-1.5 pl-4 text-[13px] leading-relaxed text-zinc-700 dark:text-zinc-300">
          <li>
            <span className="text-cyan-700 dark:text-cyan-300">Jobs arrive faster:</span> every Kimedics job email becomes a
            Salesforce record in a median of {`${KIM_LATENCY.medianMin} minutes`}, around the clock —
            before, it waited on a person&apos;s inbox.
          </li>
          <li>
            <span className="text-emerald-700 dark:text-emerald-300">Candidate intake runs itself:</span> {julAutoShare}% of all
          ...
          <li>
            <span className="text-amber-700 dark:text-amber-300">Placements haven&apos;t risen yet</span> —{' '}
          ...
          <li>
            <span className="text-zinc-900 dark:text-zinc-100">What&apos;s already banked:</span> ~{totalHours} hours of manual
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Hero big={`${KIM_LATENCY.medianMin} min`} label="email → Salesforce job" detail={`median of ${KIM_LATENCY.n.toLocaleString()} emails · 90% within ${KIM_LATENCY.p90Min} min`} accent="text-cyan-700 dark:text-cyan-300" />
        <Hero big={`${julAutoShare}%`} label="of candidate inflow automated" detail={`${djc.created} contacts created since Jun 16, all deduped`} accent="text-emerald-700 dark:text-emerald-300" />
        <Hero big={`${v.sameWindow.y2026} vs ${v.sameWindow.y2025}`} label="placements vs same weeks '25" detail="flat so far — see section 3 for the why" accent="text-amber-700 dark:text-amber-300" />
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          >
            ...
            <div className="mt-3 flex flex-wrap gap-x-8 gap-y-2">
              <BigPair v={`${KIM_LATENCY.medianMin} min`} l="email → record (median)" accent="text-cyan-700 dark:text-cyan-300" />
            ...
            <p className="mt-3 text-[12px] leading-relaxed text-zinc-600 dark:text-zinc-400">
              ...
              <span className="text-zinc-800 dark:text-zinc-200">{KIM_JOB_WORK.created}</span> outright and{' '}
              <span className="text-zinc-800 dark:text-zinc-200">{KIM_JOB_WORK.worksitesCreated}</span> worksites — most
        ...
        <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={s.label} className="flex items-center gap-3">
                  <span className="w-40 shrink-0 text-[12px] text-zinc-700 dark:text-zinc-300">{s.label}</span>
          ...
          <Card title="The honest verdict: not yet">
            <div className="flex flex-wrap gap-x-8 gap-y-3">
              <BigPair v={String(v.sameWindow.y2026)} l={`placements, first ${v.postDays} automation days`} accent="text-amber-700 dark:text-amber-300" />
            ...
            <p className="mt-4 text-[12px] leading-relaxed text-zinc-600 dark:text-zinc-400">
              ...
              is <span className="text-zinc-800 dark:text-zinc-200">not statistically significant</span> and overlaps the
          ...
          <Card title="Why that's the expected result — and what to watch">
            <ul className="space-y-3 text-[12px] leading-relaxed text-zinc-700 dark:text-zinc-300">
              <li>
                <span className="font-semibold text-zinc-900 dark:text-zinc-100">The mechanism is slow by nature.</span>{' '}
              ...
              <li>
                <span className="font-semibold text-emerald-700 dark:text-emerald-300">The early signals are moving:</span>{' '}
              ...
              <li>
                <span className="font-semibold text-zinc-900 dark:text-zinc-100">The scoreboard to watch:</span> quarterly
            ...
            <Link href="/djc/pipeline" className="mt-3 inline-block text-[11px] text-cyan-600 dark:text-cyan-400 hover:underline">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          >
              ...
              {DJC_TIME_TASKS.map(task => {
                ...
                return (
                  <div key={task.label} className="flex items-center gap-3">
                    <span className="w-48 shrink-0 text-[11px] leading-tight text-zinc-600 dark:text-zinc-400">
                      {task.label}
                      {'note' in task && task.note && (
                        <span className="block text-[10px] text-zinc-500 dark:text-zinc-600">{task.note}</span>
                      )}
                    </span>
                    <span className="relative h-4 grow rounded bg-zinc-200/70 dark:bg-zinc-800/50">
                      <span className="absolute inset-y-0 left-0 rounded bg-cyan-500/70 dark:bg-cyan-400/60"
            ...
            <div className="mt-4 border-t border-zinc-200 dark:border-zinc-800 pt-3">
              <p className="mb-2 text-[11px] uppercase tracking-wide text-zinc-500 dark:text-zinc-600">Hours returned per month</p>
              <div className="flex items-end gap-2">
                {DJC_HOURS_MONTHLY.map(m => {
                  const max = Math.max(...DJC_HOURS_MONTHLY.map(x => x.hours), 1)
                  return (
                    <div key={m.month} className="flex flex-1 flex-col items-center gap-1">
                      <span className="text-[11px] font-semibold tabular-nums text-zinc-900 dark:text-zinc-100">{m.hours}h</span>
            ...
            <p className="mt-3 text-[12px] leading-relaxed text-zinc-600 dark:text-zinc-400">
              ...
              <span className="font-semibold text-zinc-900 dark:text-zinc-100">~{kim.hoursSaved + Math.round(DJC_HOURS_TOTAL)} hours</span>{' '}
      ...
      <section>
        ...
        <Card title="Methodology & honest caveats">
          <ul className="list-disc space-y-2 pl-4 text-[12px] leading-relaxed text-zinc-600 dark:text-zinc-400">
            <li>
              <span className="text-zinc-700 dark:text-zinc-300">Job speed</span>: per-email measurement (email&apos;s own
            ...
            <li>
              <span className="text-zinc-700 dark:text-zinc-300">Candidate inflow share</span>: Salesforce contact creations
              with DJC origin, by month, attributed to the automation via its own creation log
              ({julAutoShare}% in July).
            </li>
            <li>
              <span className="text-zinc-700 dark:text-zinc-300">The placement verdict</span> uses two controls: the same
            ...
            <li>
              <span className="text-zinc-700 dark:text-zinc-300">Hours</span>: disclosed per-action minutes × actual logged
            ...
            <li>
              <span className="text-zinc-700 dark:text-zinc-300">What we don&apos;t claim</span>: the Q2 2026 placement record
...
function Hero({ big, label, detail, accent }: { big: string; label: string; detail: string; accent?: string }) {
  return (
    <div className="min-w-0 rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700/50 dark:bg-zinc-800/30 dark:shadow-none p-4 sm:p-5">
      <div className={`text-2xl font-semibold leading-none tabular-nums sm:text-3xl ${accent ?? 'text-zinc-900 dark:text-zinc-100'}`}>{big}</div>
  ...
  return (
    <div className="mb-4">
      <div className="flex items-center gap-3">
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-cyan-300 bg-cyan-50 dark:border-cyan-700/50 dark:bg-cyan-950/40 text-[11px] font-semibold text-cyan-700 dark:text-cyan-300">
          {n}
        </span>
        <h2 className="min-w-0 text-[15px] font-semibold text-zinc-900 dark:text-zinc-100">{title}</h2>
      </div>
...
function BigPair({ v, l, accent }: { v: string; l: string; accent?: string }) {
  return (
    <div className="min-w-0">
      <div className={`text-2xl font-semibold tabular-nums ${accent ?? 'text-zinc-900 dark:text-zinc-100'}`}>{v}</div>
...
function ModelRow({ n, unit, label, color, frac, max }: {
  ...
  return (
    <div>
      ...
      <div className="mt-1 h-1.5 rounded-sm bg-zinc-200 dark:bg-zinc-800">
...
function StackedColumns({ series, colorA, colorB, legendA, legendB }: {
  ...
  return (
    <div>
      <div className="mb-2 flex items-center gap-4 text-[11px] text-zinc-600 dark:text-zinc-400">
