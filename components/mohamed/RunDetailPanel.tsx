...
const stageChip: Record<string, string> = {
  ...
  not_run: 'bg-zinc-50 text-zinc-400 border-zinc-200 dark:bg-zinc-900/50 dark:text-zinc-500 dark:border-zinc-800',
}
...
export function RunDetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-50" role="dialog" aria-modal="true" aria-label="Run detail">
      ...
      <aside className="absolute inset-y-0 right-0 flex w-full max-w-xl flex-col overflow-y-auto bg-white dark:bg-zinc-900 shadow-2xl lg:max-w-2xl">
        <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 px-5 py-3">
          <div>
          ...
          <button type="button" onClick={onClose} className="rounded-md px-2 py-1 text-xs text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 dark:hover:bg-zinc-800 dark:hover:text-zinc-100">
            Close ✕
          </button>
        </div>
        ...
        <div className="flex-1 px-5 py-4">
          ...
          {ledger && badge && (
            <div className="space-y-4">
              ...
              <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">{summariseInPlainLanguage(ledger)}</p>
              ...
              {gapAlert && (
                <div className="rounded-xl border border-amber-200 bg-amber-50/60 dark:border-amber-500/30 dark:bg-amber-500/10 p-4">
                  <div className="flex items-start gap-2">
                    <span className="mt-0.5 shrink-0 text-base text-amber-600 dark:text-amber-400" aria-hidden>ⓘ</span>
                    <div>
              ...
              <div>
                <p className="mb-2 text-xs font-medium text-zinc-500 dark:text-zinc-400">Stages</p>
              ...
              <div>
                ...
                {claims.length === 0 && <p className="text-xs text-zinc-400 dark:text-zinc-500">No claims in this run.</p>}
                {reviewable.length > 0 && (
                  <ClaimsByMember runId={ledger.run_id} claims={reviewable} approvals={approvals} canApprove={canApprove} />
                )}
                {notReviewable.length > 0 && (
                  <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200 px-4 py-3 text-xs">
        ...
        <div className="sticky bottom-0 border-t border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 px-5 py-3 text-xs text-zinc-500">
          <a href={`/mohamed?run=${runId}`} className="font-medium text-emerald-700 dark:text-emerald-400 hover:underline">
            Open as full page →
          </a>
